...
def main():
    ...
    try:
        ...
        # Pass target_date to update_inventory_sheet so it knows which date to use
        update_inventory_sheet(sales_data, target_date=target_date)
...
def fetch_clover_sales(creds, target_date=None):
    ...
    try:
        ...
        params = {
            'access_token': token,
            'filter': f'createdTime>={extended_start_ms}',  # Use extended range
            'expand': 'lineItems',
        ...
        for order in filtered_orders:
            ...
            if order_state in ['locked', 'paid', 'open', 'completed']:
                ...
                for item in line_items:
                    ...
                    # ENHANCED: Explicitly check for Brookie FIRST before other checks
                    # This ensures all Brookie items are captured regardless of other patterns
                    is_brookie = 'brookie' in item_name_lower and 'F' in item_name
                    
                    is_cookie = (
                        is_brookie or  # Check Brookie FIRST
                    ...
                    # DEBUG: Log if Brookie item is being filtered out
                    if 'brookie' in item_name_lower:
                        if 'VSJ' in creds.get('name', '') or 'Old San Juan' in creds.get('name', ''):
                            if not is_cookie:
                                brookie_filtered_count += quantity
                                logging.warning(f"⚠️ VSJ BROOKIE ITEM FILTERED OUT: '{item_name}' - is_cookie={is_cookie}")
                            else:
                                brookie_processing_count += quantity
                    ...
                    if is_cookie:
                        ...
                        # Special debug logging for F cookies (Brookie)
                        if 'F' in item_name and 'brookie' in item_name_lower:
                            logging.info(f"🎯 F BROOKIE DEBUG: {item_name} - qty: {quantity}, running total: {cookie_sales[item_name]}")
...
def update_inventory_sheet(sales_data, target_date=None):
    ...
    try:
        ...
        # Additional sheet: Drunken Cookies summary - same data, also updated each run.
        DRUNKEN_COOKIES_SHEET_ID = "1OrhmZgQRbMpzewqvdQd6Wipv-sIC4s1gEw9aJO-ldgE"
        additional_sheet_ids = [DRUNKEN_COOKIES_SHEET_ID]
        ...
        # Update Drunken Cookies sheet early (so backfill always writes even if primary validation fails later)
        # Use check_date (which respects FOR_DATE or target_date parameter) for Drunken Cookies update
        for extra_sheet_id in additional_sheet_ids:
            try:
                update_drunken_cookies_sheet(
                    service, extra_sheet_id, sales_data, check_date, desired_tab
                )
            except Exception as ex:
                logging.warning(f"⚠️ Could not update Drunken Cookies sheet {extra_sheet_id[:20]}...: {ex}")
...
def clean_cookie_name(api_name):
    ...
    name_mapping = {
        ...
        "Brookie": "F - Brookie",  # Add Brookie mapping
    ...
    # This helps automatically detect new flavors that aren't in the mapping yet
    best_match_score = 0
    best_match_name = None
    
    for api_pattern, sheet_name in name_mapping.items():
        # Use fuzzy matching to find similar cookie names
        from fuzzywuzzy import fuzz
        score = fuzz.partial_ratio(cleaned.lower(), api_pattern.lower())
        if score > best_match_score and score >= 75:  # 75% similarity threshold
        ...
        try:
            ...
            for h in cookie_headers:
                ...
                for api_cookie, count in cookies.items():
                    ...
                    base = _strip_cookie_prefix(cleaned)
                    if _normalize_for_match(base) == h_norm or _normalize_for_match(cleaned) == h_norm:
                        val = count
                        matched_cookies.add(api_cookie)
                        break
                    if h_norm in _normalize_for_match(base) or _normalize_for_match(base) in h_norm:
                        val = count
                        matched_cookies.add(api_cookie)
                        break
                row_vals.append(val)
            ...
            for api_cookie, count in cookies.items():
                ...
                if display_name and count > 0:
                    new_headers.append((display_name, count))
            if new_headers:
