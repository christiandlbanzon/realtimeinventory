...
def fetch_clover_category_items(creds):
    ...
    Returns set of (item_id, item_name) for items in the category.
    """
    ...
    try:
        ...
        return allowed_ids, id_to_name
    except Exception as e:
        logging.warning(f"Error fetching category items: {e}")
        return set(), {}


# Items that are NOT cookies - exclude from counts and sheet even if in Clover cookie category
NON_COOKIE_KEYWORDS = frozenset([
    ...
    # Non-cookie desserts
    'red velvet cake', 'cherry red velvet', 'turron', 'sticky toffee pudding',
    # Test / empty
...
def fetch_clover_sales(creds, target_date=None):
    ...
    try:
        ...
        allowed_item_ids, _ = fetch_clover_category_items(creds)
        ...
        for order in filtered_orders:
            ...
            if order_state in ['locked', 'paid', 'open', 'completed']:
                ...
                cookie_keywords = ['cookie', 'chocolate', 'nutella', 'cheesecake', 'churro', 'tres leches', 'fudge', 's\'mores', 'cinnamon', 'lemon', 'strawberry', 'pecan', 'guava', 'macadamia', 'biscoff', 'brookie', 'brownie', 'cornbread', 'cherry', 'cake']
                ...
                for line_item in line_items:
                    ...
                    if allowed_item_ids:
                        # Use category items list - definitive from Clover Item list
                        if item_id and item_id in allowed_item_ids:
                            # Secondary filter: exclude non-cookies even if miscategorized in Clover
                            if _is_non_cookie_by_name(item_name):
                                continue  # Merchandise, flawed, expired, etc.
                            pass  # Item is in cookie category and passes name filter - count it
                        else:
                            continue  # Not in cookie category - skip (M, Q, R, etc.)
...
def update_inventory_sheet(sales_data, target_date=None, clover_creds=None):
    ...
    try:
        ...
        if default_sheet_id:
            ...
            # Fallback: hardcoded IDs for sheets not in folder or if folder lookup fails
            if current_month == 1:
        ...
        logging.info(f"📅 Using date: {target_date.strftime('%Y-%m-%d')} -> tab: {desired_tab}")
        ...
        for location, sales in sales_data.items():
            ...
            if location in location_mapping:
                ...
                if sheet_location in location_columns:
                    ...
                    # Fetch allowed items from Clover - clear rows NOT in cookie category (M, Q, R, etc.)
                    # Exclude non-cookies (merchandise, flawed, expired, etc.) even if in Clover category
                    allowed_sheet_names = set()
                    if clover_creds and location in clover_creds:
                        _, id_to_name = fetch_clover_category_items(clover_creds[location])
                        allowed_sheet_names = {
                            clean_cookie_name(n) for n in id_to_name.values()
                            if not _is_non_cookie_by_name(n)
                        }
                    ...
                    for cookie_name, sales_count in sales.items():
                        # Skip non-cookies (merchandise, flawed, expired, etc.)
                        if _is_non_cookie_by_name(cookie_name):
                            logging.info(f"Skip non-cookie from sheet: {cookie_name}")
                            continue
                        ...
                        for i, sheet_cookie in enumerate(cookie_names):
                            ...
                            if cleaned not in allowed_sheet_names:
                                ...
                                if exp_col is not None:
                                    exp_cell = f"{sheet_tab}!{column_to_letter(exp_col)}{cookie_row}"
                                ...
                                # Reset row label in column A to [NOT IN USE] so formulas treat as unused
                                label_cell = f"{sheet_tab}!A{cookie_row}"
                                if label_cell not in updates_by_cell:
                                    updates.append({'range': label_cell, 'values': [["[NOT IN USE]"]]})
                                    updates_by_cell[label_cell] = "[NOT IN USE]"
                                    logging.info(f"🧹 Reset row label to [NOT IN USE] for {sheet_cookie} at {label_cell}")
...
def find_or_claim_not_in_use_row(service, sheet_id, sheet_tab, cookie_names, api_cookie_name):
    """
    When a new Clover flavor has no matching sheet row, claim a [NOT IN USE]
    slot in the correct alphabetical position (e.g. J after I, before K).
    Uses the letter from Clover (J - Churro with Caramel) and places it correctly.
    """
    if 'S:Jalda' in api_cookie_name or 'jalda' in api_cookie_name.lower():
        return None
    # Get display name and letter from Clover
    cleaned = clean_cookie_name(api_cookie_name)
    base_name = _strip_cookie_prefix(cleaned) or cleaned or api_cookie_name
    if not base_name or len(base_name) < 3:
        return None
    # Extract letter from Clover (e.g. "J" from "J - Churro with Caramel" or "*J* Churro with Caramel")
    letter_match = re.match(r'^([A-Z])\s*-\s*', cleaned, re.IGNORECASE)
    if not letter_match:
        letter_match = re.search(r'\*([A-Z])\*', api_cookie_name, re.IGNORECASE)
    target_letter = letter_match.group(1).upper() if letter_match else None
    if not target_letter:
        # Fallback: use next available letter
        used_letters = {m.group(1).upper() for c in cookie_names if c and (m := re.match(r'^([A-Z])\s*-\s*', str(c), re.IGNORECASE))}
        for letter in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
            if letter not in used_letters:
                target_letter = letter
                break
    if not target_letter:
        logging.warning(f"⚠️ No available letter for new flavor: '{api_cookie_name}'")
        return None
    new_label = f"{target_letter} - {base_name}"
    # Find [NOT IN USE] row in correct position: between prev letter (I) and next letter (K)
    def get_letter(c):
        if not c:
            return None
        m = re.match(r'^([A-Z])\s*-\s*', str(c), re.IGNORECASE)
        return m.group(1).upper() if m else None
    prev_letter = chr(ord(target_letter) - 1) if target_letter != 'A' else None
    next_letter = chr(ord(target_letter) + 1) if target_letter != 'Z' else None
    # Find index of last row with prev_letter (e.g. last I) and first row with next_letter (e.g. first K)
    last_prev_idx = None
    first_next_idx = None
    for i, c in enumerate(cookie_names):
        L = get_letter(c)
        if L == prev_letter:
            last_prev_idx = i
        if L == next_letter and first_next_idx is None:
            first_next_idx = i
    # Pick [NOT IN USE] between last_prev_idx and first_next_idx (or first available as fallback)
    best_i = None
    for i, cookie in enumerate(cookie_names):
        if not cookie or '[NOT IN USE]' not in str(cookie).upper():
            continue
        # Ideal: between last_prev_idx and first_next_idx
        if last_prev_idx is not None and i <= last_prev_idx:
            continue
        if first_next_idx is not None and i >= first_next_idx:
            continue
        best_i = i
        break
    if best_i is None:
        # Fallback: first [NOT IN USE] anywhere
        for i, cookie in enumerate(cookie_names):
            if cookie and '[NOT IN USE]' in str(cookie).upper():
                best_i = i
                break
    if best_i is None:
        logging.warning(f"⚠️ No [NOT IN USE] slot available for new flavor: '{api_cookie_name}'")
        return None
    i = best_i
    sheet_row = i + 3  # cookie_names[i] is at sheet row i+3
    cell_range = f"'{sheet_tab}'!A{sheet_row}"
    try:
        service.spreadsheets().values().update(
            spreadsheetId=sheet_id,
            range=cell_range,
            valueInputOption='USER_ENTERED',
            body={'values': [[new_label]]}
        ).execute()
        cookie_names[i] = new_label
        logging.info(f"🆕 Claimed [NOT IN USE] row {sheet_row} for new flavor: '{new_label}' (from Clover: '{api_cookie_name}')")
        return sheet_row
    except Exception as e:
        logging.error(f"❌ Failed to claim [NOT IN USE] row for '{api_cookie_name}': {e}")
        return None
        ...
        try:
            ...
            for api_cookie, count in cookies.items():
                if api_cookie in matched_cookies:
                    continue
                if _is_non_cookie_by_name(api_cookie):
                    continue  # Don't add merchandise, flawed, expired, etc. as columns
                cleaned = clean_cookie_name(api_cookie)
