#!/usr/bin/env python3
"""
Check all columns that might be related to OSJ/Old San Juan
to find where the 490 value is
"""
...
def main():
    ...
    print()
    print("Checking all columns for OSJ/Old San Juan/VSJ:")
    print("="*80)
    
    # Check all columns
    osj_columns = []
    for i in range(min(len(location_row), len(headers), 100)):  # Check first 100 columns
        location_name = str(location_row[i]).strip() if i < len(location_row) else ""
        header = str(headers[i]).strip() if i < len(headers) else ""
        
        # Check if this column is related to OSJ
        if any(keyword in location_name.lower() for keyword in ['old san juan', 'osj', 'vsj']) or \
           any(keyword in header.lower() for keyword in ['old san juan', 'osj', 'vsj']):
    ...
    print("="*80)
    print(f"Found {len(osj_columns)} OSJ-related columns with values:")
    for col_info in osj_columns:
