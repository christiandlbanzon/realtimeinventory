...
def update_inventory_sheet(sales_data):
    ...
    try:
        ...
        sheet_id = os.getenv("INVENTORY_SHEET_ID", "1IoWQwykXFe7zeDgfCBc7C0ti7TrDB0GGBHMgLmM2Xno")
...
if [ -f vm_inventory_updater.py ]; then
    ...
    if grep -q "FIX: Only use fallback if API actually failed" vm_inventory_updater.py; then
        echo "✅ San Patricio fix found"
    else
        echo "❌ San Patricio fix NOT found"
    fi
    if grep -q "LARGE DISCREPANCY DETECTED" vm_inventory_updater.py; then
        echo "✅ Validation fix found"
    else
        echo "❌ Validation fix NOT found"
    fi
else
